# Sugestão de resolução do exercício da sessão 4. Obter informação a partir do ficheiro do SINAVE.
...
def contagens (ficheiro):
    ...
    O número de casos notificados estão dispostos por linhas, sendo que o delimitador é um (;).
    ...
    index = 0 # Inicialização da variável "index", que vai permitir identificar o índice de cada elemento da lista "headers" correspondente ao cabeçalho da tabela. 
...
contagens("doencas-de-declaracao-obrigatoria.csv") # Nome do ficheiro ou caminho completo para o ficheiro
